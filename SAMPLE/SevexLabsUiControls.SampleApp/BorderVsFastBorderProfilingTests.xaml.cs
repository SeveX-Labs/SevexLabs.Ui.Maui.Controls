...
public partial class BorderVsFastBorderProfilingTests : ContentPage
{
    ...
    private async void OnRunInlineComparisonClicked(object? sender, TappedEventArgs e)
    {
        if (_isRunning)
            return;
    ...
    private async void OnOpenBorderPageClicked(object? sender, TappedEventArgs e)
    {
        await OpenProfilingPageAsync(BorderProfilingTarget.Border, autoPop: false);
    }

    private async void OnOpenFastBorderPageClicked(object? sender, TappedEventArgs e)
    {
        await OpenProfilingPageAsync(BorderProfilingTarget.FastBorder, autoPop: false);
    }

    private async void OnRunNavigationComparisonClicked(object? sender, TappedEventArgs e)
    {
        if (_isRunning)
            return;
    ...
    private void OnClearLogClicked(object? sender, TappedEventArgs e)
    {
        _logEntries.Clear();
